...
NodABC* inserare_nod_ABC(NodABC* r, CardBancar data, unsigned char* inserat) {
	...
	{
	//exista nod care trebuie verificat in raport cu cheia din data
		if (strcmp(data.nr_card,r->cb.nr_card) < 0) {
			//se continua cautarea pe descendentul stanga
			...
			if (strcmp(data.nr_card, r->cb.nr_card) > 0) {
				...
				//cheie de inserat este identificata in arbore
				//se abandoneaza op de inserare
				*inserat = 0;
			}
		}
	}
	else {
		//locul de inserat a fost identificat in arbore(este pozitie de NULL in structura ABC)
...
int main()
{
	...
	while (!feof(f))
	{
		...
		if (inserat != 0) {
			printf("Inserare cu succes %s\n", card.nr_card);
		}
		else {
			printf("Inserare esuata card %s\n", card.nr_card);
			//dezalocare extensii card in mem heap
			free(card.emitent);
			free(card.titular);
		}
	...
	printf("Arbore binar de cautare traversat Inordine\n");
