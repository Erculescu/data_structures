...
//functia care calculeaza maximul soldurilor din abc

void calculeaza_maxim(NodABC* r, float* max_sold) {
	if (r != NULL) {
		if (r->cb.sold > *max_sold) {
			*max_sold = r->cb.sold;
		}

	}
	if (r->stanga != NULL) {
		calculeaza_maxim(r->stanga, max_sold);
	}
	if (r->dreapta != NULL) {
		calculeaza_maxim(r->dreapta, max_sold);
	}
}

//functie care creeaza un vector de carduri cu soldul maxim
void creare_vector(CardBancar** carduri, int* dim, NodABC* r, float max_sold) {
	if (r != NULL) {
		if (r->cb.sold == max_sold) {
			CardBancar* card_temp = realloc(*carduri, (*dim + 1) * sizeof(CardBancar));
			if (card_temp == NULL) {
				return;
			} 
			*carduri = card_temp;
			...
			*dim += 1;
			...
			creare_vector(carduri, dim, r->stanga, max_sold);				
			creare_vector(carduri, dim, r->dreapta, max_sold);		
	}
}

//functia care salveaza intr-un vector carduri bancare cu sold maxim in ABC
CardBancar** creare_vector_v2(int* dim, NodABC* r) {
	float max = 0.0f;
	calculeaza_maxim(r, &max);
	CardBancar* carduri = NULL;
	creare_vector(&carduri, dim, r, max);
	return carduri;
}
...
int main()
{
	...
	float max_sold = 0.0f;
	calculeaza_maxim(root, &max_sold);
	printf("\nSold maxim: %.2f\n", max_sold);
	CardBancar* carduri = NULL;
	int dim = 0;
	carduri = creare_vector_v2(&dim, root);
	for (int i = 0; i < dim; i++) {
		printf("\n%s ",carduri[i].nr_card);
	}
